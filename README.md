...
| 原生视觉持续同步 | 未指定模板或自定义视觉时，使用当前 Codex 原生 Presentations 的模板选择与默认布局体系；不会把历史白底深蓝风格永久固化为默认。 |
...
7. 新建科研 PPTX 默认由 PPT Agent 隔离生产上下文并统筹制作，底层仍使用当前内置 Presentations。视觉路线按“参考模板 → 明确自定义 → Codex 原生默认”的顺序确定；局部修改或简单短稿可以直接使用内置工具。公式较多时先试制一条代表性公式。
...
所有页面都需要渲染后检查。标题采用简洁、自然的主题型表达；讲述句和汇报组织说明进入正文或演讲者备注。未指定模板或明确自定义视觉时，使用当前 Codex 原生 Presentations 的模板选择与默认布局体系；白底、深灰和克制深蓝作为可选的自定义学术风格保留，而不再作为强制默认。
