...
4. 建立临时证据台账，区分权威角色以及已验证、未验证、计划、受阻、否决和待确认内容。
5. 提交事实摘要、冲突项与逐页大纲，等待确认。
6. 推荐并披露制作路径：PPT Agent、内置演示工具、`oil-ppt` 或明确说明的自定义流程。
7. 生成演示文稿，检查内容边界、章节过渡、公式比例、备注、溢出和视觉一致性。
8. 默认只交付 `<YYYY-MM-DD>_<汇报类型>.pptx`。
